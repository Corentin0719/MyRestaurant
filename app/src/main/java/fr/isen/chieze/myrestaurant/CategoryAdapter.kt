...
class CategoryAdapter(val  items: List<Items>, val mListener: (Items) -> Unit)  : RecyclerView.Adapter<CategoryAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CategoryAdapter.ViewHolder, position: Int) {
        ...
        Picasso.get().load(menu.images[0].ifEmpty { null }).error(R.drawable.plat).into(holder.dishImage)
    ...
    class ViewHolder(ItemView: View) : RecyclerView.ViewHolder(ItemView) {
        ...
        val dishName: TextView = ItemView.findViewById(R.id.nameDish)
        val dishPrice: TextView = ItemView.findViewById(R.id.priceDish)
        val dishImage : ImageView = ItemView.findViewById(R.id.pictureDish)
    }
}
